...
fn parse(input: &str) -> Cave {
    ...
    // Push empty roow since room idx begins at 1
...
pub fn part_a(input: &str) -> usize {
    // https://github.com/synapticarbors/advent_of_code_2022/blob/main/rust/aoc16/src/main.rs
    // I will learn it later.
    let cave = parse(input);
    let distances = cave.calc_distances();
    let mut targets: HashSet<usize> = HashSet::from_iter(
        cave.rooms
            .iter()
            .enumerate()
...
mod tests {
    ...
    fn part_a_works() {
        ...
        assert_eq!(part_a(input), 1651);
    }

    #[test]
    fn part_a_result() {
        let input = include_str!("input.txt");
        assert_eq!(part_a(input), 2087);
    }

    #[test]
    fn part_b_result() {
        let input = include_str!("input.txt");
        assert_eq!(part_b(input), 2591);
    }
}
